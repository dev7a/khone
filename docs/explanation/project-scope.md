...
The practical consequence is that benchmark results should be read as scenario measurements rather than global guarantees. Workloads with different traffic distribution, target duration, payload size, or scale-out behavior can produce different batch sizes and latency tradeoffs.
